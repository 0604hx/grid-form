...
<template>
    <div :class="cls" :style="{backgroundColor, padding, margin:'5px 5px 0px 5px', borderRadius:'4px', position:'relative', display: show?'':'none'}">
        <var-icon v-if="showIcon" :name="icon" size="1.7rem" :color="colors[type]" style="display: flex;position: absolute; top: 12px;" />
        <var-icon v-if="closeable" name="window-close" style="position: absolute; top:18px;right:15px; cursor: pointer;" @click="show=false" />
...
<script setup>
    ...
    const icons = { success:"checkbox-marked-circle", info:"information", error:"close-circle" }
